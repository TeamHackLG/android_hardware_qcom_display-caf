    ...
    if(connected != -1) { //either we got switch_state connected or disconnect
        ...
        if (connected) {
            ...
            ctx->mFBUpdate[HWC_DISPLAY_EXTERNAL] =
                ...
                if(ctx->mMDPComp[HWC_DISPLAY_EXTERNAL]) {
                    delete ctx->mMDPComp[HWC_DISPLAY_EXTERNAL];
                    ctx->mMDPComp[HWC_DISPLAY_EXTERNAL] = NULL;
                }
            if(usecopybit)
            ...
            if(ctx->mCopyBit[HWC_DISPLAY_EXTERNAL]){
                ...
                ctx->mMDPComp[HWC_DISPLAY_EXTERNAL] =  MDPComp::getObject(
                        ctx->dpyAttr[HWC_DISPLAY_EXTERNAL].xres, HWC_DISPLAY_EXTERNAL);
        }
